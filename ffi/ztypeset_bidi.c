//===----------------------------------------------------------------------===//
// ztypeset -- the Unicode Bidirectional Algorithm and script itemisation.
//
// Both come from SheenBidi, and both stop at runs: what ztypeset produces is
// the input a shaper needs, not a laid-out line.
//
...
// describe. That is what ZtypesetLine is for; ZtypesetParagraph's runs are the
...
static SBLevel toSbBaseLevel(ZtypesetBaseDirection base) {
  switch (base) {
    case ZTYPESET_BASE_DIRECTION_LTR:
      return 0;
    case ZTYPESET_BASE_DIRECTION_RTL:
      return 1;
    case ZTYPESET_BASE_DIRECTION_AUTO:
    default:
...
static ZtypesetResult runsFromLine(ZtypesetAllocatorId owner, SBLineRef line,
                                ZtypesetArray* out) {
  const SBUInteger run_count = SBLineGetRunCount(line);
  if (run_count == 0u) return ZTYPESET_RESULT_OK;

  const SBRun* runs = SBLineGetRunsPtr(line);
  if (!ztypesetArrayReserve(owner, out, (size_t)run_count,
                         sizeof(ZtypesetVisualRun))) {
    return ZTYPESET_RESULT_OUT_OF_MEMORY;
  }

  ZtypesetVisualRun* dst = (ZtypesetVisualRun*)out->data;
...
static ZtypesetResult collectScriptRuns(ZtypesetParagraph* paragraph,
                                     const SBCodepointSequence* sequence) {
  SBScriptLocatorRef locator = SBScriptLocatorCreate();
  if (locator == NULL) return ZTYPESET_RESULT_OUT_OF_MEMORY;
  ...
  while (SBScriptLocatorMoveNext(locator) != SBFalse) {
    if (!ztypesetArrayReserve(ztypesetAllocatorOf(paragraph),
                           &paragraph->script_runs,
                           paragraph->script_runs.count + 1u,
                           sizeof(ZtypesetScriptRun))) {
      result = ZTYPESET_RESULT_OUT_OF_MEMORY;
      break;
    }
    ZtypesetScriptRun* out = (ZtypesetScriptRun*)paragraph->script_runs.data;
    ZtypesetScriptRun* slot = &out[paragraph->script_runs.count];
    ...
    // is exactly what ZtypesetShapeParams::script takes. SheenBidi also offers
    // an OpenType tag, which is a different thing and would be wrong here.
...
static ZtypesetResult intersectRuns(ZtypesetAllocatorId owner,
                                 const ZtypesetVisualRun* visual,
                                 size_t visual_count,
                                 const ZtypesetScriptRun* scripts,
                                 size_t script_count, ZtypesetArray* out) {
  for (size_t v = 0u; v < visual_count; v++) {
    ...
    if (first == script_count) {
      if (!ztypesetArrayReserve(owner, out, out->count + 1u,
                             sizeof(ZtypesetShapingRun))) {
        return ZTYPESET_RESULT_OUT_OF_MEMORY;
      }
      ZtypesetShapingRun* dst = (ZtypesetShapingRun*)out->data;
...
/// paragraph's scripts. This is the whole of both ztypesetParagraphCreate's
/// run collection and ztypesetLineCreate's -- the difference between them is
/// only which range is asked for.
static ZtypesetResult runsForRange(ZtypesetAllocatorId owner,
                                SBParagraphRef sb_paragraph, size_t offset,
                                size_t length, const ZtypesetScriptRun* scripts,
                                size_t script_count, ZtypesetArray* visual_out,
                                ZtypesetArray* shaping_out) {
  ...
  if (line == NULL) return ZTYPESET_RESULT_OUT_OF_MEMORY;

  ZtypesetResult result = runsFromLine(owner, line, visual_out);
  SBLineRelease(line);
  if (result != ZTYPESET_RESULT_OK) return result;

  return intersectRuns(owner, (const ZtypesetVisualRun*)visual_out->data,
...
// state, so all ztypeset supplies is the buffer and the translation of
// upstream's per-algorithm constants into one ZtypesetBreak.
//===----------------------------------------------------------------------===//

/// libunibreak's line-break codes, which have their own numbering per
/// algorithm, translated into the one enum ztypeset exposes.
static uint8_t fromLineBreak(char code) {
  switch (code) {
    case LINEBREAK_MUSTBREAK:
      return ZTYPESET_BREAK_MANDATORY;
    case LINEBREAK_ALLOWBREAK:
      return ZTYPESET_BREAK_ALLOWED;
...
static uint8_t fromClusterBreak(char code) {
  return code == 0 ? ZTYPESET_BREAK_ALLOWED : ZTYPESET_BREAK_NONE;
}
...
static void segment(const ZtypesetParagraph* paragraph) {
...
static ZtypesetResult collectBreaks(ZtypesetParagraph* paragraph) {
  const size_t length = paragraph->length;
  const uint32_t want = paragraph->segmentation;
  if (length == 0u || want == (uint32_t)ZTYPESET_SEGMENTATION_NONE) {
    return ZTYPESET_RESULT_OK;
  }

  size_t arrays = 0u;
  if ((want & (uint32_t)ZTYPESET_SEGMENTATION_LINES) != 0u) arrays++;
  if ((want & (uint32_t)ZTYPESET_SEGMENTATION_GRAPHEMES) != 0u) arrays++;
  if ((want & (uint32_t)ZTYPESET_SEGMENTATION_WORDS) != 0u) arrays++;
  ...
  if (length > SIZE_MAX / arrays) return ZTYPESET_RESULT_OUT_OF_MEMORY;
  paragraph->breaks =
      (uint8_t*)ztypesetAlloc(length * arrays, ZTYPESET_DEFAULT_ALIGN);
  if (paragraph->breaks == NULL) return ZTYPESET_RESULT_OUT_OF_MEMORY;

  uint8_t* next = paragraph->breaks;
  if ((want & (uint32_t)ZTYPESET_SEGMENTATION_LINES) != 0u) {
    paragraph->line_breaks = next;
    next += length;
  }
  if ((want & (uint32_t)ZTYPESET_SEGMENTATION_GRAPHEMES) != 0u) {
    paragraph->grapheme_breaks = next;
    next += length;
  }
  if ((want & (uint32_t)ZTYPESET_SEGMENTATION_WORDS) != 0u) {
    paragraph->word_breaks = next;
  }

  // libunibreak writes one char per input code unit and reads nothing else,
  // so its output buffer can be ztypeset's own storage rather than a staging
  // copy.
  segment(paragraph);
...
ZtypesetResult ztypesetParagraphCreate(const void* text, size_t length,
  ...
  *out = NULL;
  ...
  const size_t unit = ztypesetEncodingUnitSize(encoding);
  if (unit == 0u) return ZTYPESET_RESULT_INVALID_ARGUMENT;
  // Run offsets and lengths are reported as uint32_t. Past that, they would
  // wrap while ztypesetParagraphLength kept returning the true size_t, and a
  // caller slicing its buffer by those offsets would read the wrong bytes --
  // or past the end. ztypeset_abi.c asserts this bound is enforced somewhere;
  // here is where.
  if (length > 0xFFFFFFFFu) return ZTYPESET_RESULT_INVALID_ARGUMENT;
  ...
  if (length > SIZE_MAX / unit) return ZTYPESET_RESULT_INVALID_ARGUMENT;
  if (!ztypesetTextIsWellFormed(text, length, encoding)) {
    return ZTYPESET_RESULT_INVALID_TEXT;
  }

  const ZtypesetResult installed = ztypesetInstallSheenbidiAllocator();
  if (installed != ZTYPESET_RESULT_OK) return installed;

  ZtypesetParagraph* paragraph = ZTYPESET_NEW(ZtypesetParagraph);
  if (paragraph == NULL) return ZTYPESET_RESULT_OUT_OF_MEMORY;
  ...
  if (length == 0u) {
    paragraph->base_level = (base == ZTYPESET_BASE_DIRECTION_RTL) ? 1u : 0u;
    *out = paragraph;
    return ZTYPESET_RESULT_OK;
  }

  // Copied before anything else, so every SheenBidi object below is built over
  // memory this paragraph owns. See ZtypesetParagraph::text for why that
  // matters. ZTYPESET_DEFAULT_ALIGN is max_align_t, so the copy is aligned for
  // uint16_t and uint32_t as well as for bytes -- which is what lets the text
  // helpers and libunibreak read it as an array of its own unit type.
  paragraph->text = (char*)ztypesetAlloc(length * unit, ZTYPESET_DEFAULT_ALIGN);
  if (paragraph->text == NULL) {
    ztypesetFree(paragraph);
    return ZTYPESET_RESULT_OUT_OF_MEMORY;
  }
  memcpy(paragraph->text, text, length * unit);

  SBCodepointSequence sequence;
  // ztypeset_abi.c asserts the three values are SheenBidi's own, which is what
  ...
  // separator describes more than one. ztypeset analyses the first and reports
  ...
  // SBParagraph retains the algorithm, so ztypeset's own reference is done
  // with.
  SBAlgorithmRelease(algorithm);
  if (paragraph->sb_paragraph == NULL) {
    ztypesetParagraphDestroy(paragraph);
    return ZTYPESET_RESULT_BIDI_FAILED;
  }
  ...
  const ZtypesetResult segmented = collectBreaks(paragraph);
  if (segmented != ZTYPESET_RESULT_OK) {
    ztypesetParagraphDestroy(paragraph);
    return segmented;
  }
  ...
  ZtypesetResult result = collectScriptRuns(paragraph, &paragraph_sequence);

  if (result == ZTYPESET_RESULT_OK) {
    result = runsForRange(ztypesetAllocatorOf(paragraph),
                          paragraph->sb_paragraph,
                          0u, (size_t)actual_length,
                          (const ZtypesetScriptRun*)paragraph->script_runs.data,
...
void ztypesetParagraphDestroy(ZtypesetParagraph* paragraph) {
  if (paragraph == NULL) return;
  if (paragraph->sb_paragraph != NULL) {
    SBParagraphRelease(paragraph->sb_paragraph);
  }
  ztypesetFree(paragraph->breaks);
  ztypesetFree(paragraph->text);
  const ZtypesetAllocatorId owner = ztypesetAllocatorOf(paragraph);
  ztypesetArrayFree(owner, &paragraph->visual_runs, sizeof(ZtypesetVisualRun));
  ztypesetArrayFree(owner, &paragraph->script_runs, sizeof(ZtypesetScriptRun));
  ztypesetArrayFree(owner, &paragraph->shaping_runs,
                    sizeof(ZtypesetShapingRun));
  ztypesetFree(paragraph);
}

size_t ztypesetParagraphLength(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? 0u : paragraph->length;
}

ZtypesetEncoding ztypesetParagraphEncoding(const ZtypesetParagraph* paragraph) {
  // UTF-8 for a NULL paragraph, like every other accessor's zero: it is the
  // enum's own zero, and there is no text to misread.
  return paragraph == NULL ? ZTYPESET_ENCODING_UTF8 : paragraph->encoding;
}

uint8_t ztypesetParagraphBaseLevel(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? 0u : paragraph->base_level;
}

const uint8_t* ztypesetParagraphLevels(const ZtypesetParagraph* paragraph) {
  if (paragraph == NULL || paragraph->sb_paragraph == NULL) return NULL;
  // SBLevel is a byte -- asserted in ztypeset_abi.c, because this cast is the
  // reason it has to stay one.
  return (const uint8_t*)SBParagraphGetLevelsPtr(paragraph->sb_paragraph);
}

uint32_t ztypesetParagraphSegmentation(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? (uint32_t)ZTYPESET_SEGMENTATION_NONE
                           : paragraph->segmentation;
}

const uint8_t* ztypesetParagraphLineBreaks(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? NULL : paragraph->line_breaks;
}

const uint8_t* ztypesetParagraphGraphemeBreaks(const ZtypesetParagraph*
                                               paragraph) {
  return paragraph == NULL ? NULL : paragraph->grapheme_breaks;
}

const uint8_t* ztypesetParagraphWordBreaks(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? NULL : paragraph->word_breaks;
}

size_t ztypesetParagraphNextGrapheme(const ZtypesetParagraph* paragraph,
                                  size_t offset) {
  ...
  const uint8_t* breaks = ztypesetParagraphGraphemeBreaks(paragraph);
  ...
  for (size_t i = offset; i < paragraph->length; i++) {
    if (breaks[i] != ZTYPESET_BREAK_NONE) return i + 1u;
  }
  return paragraph->length;
}

size_t ztypesetParagraphPreviousGrapheme(const ZtypesetParagraph* paragraph,
                                      size_t offset) {
  const uint8_t* breaks = ztypesetParagraphGraphemeBreaks(paragraph);
...
size_t ztypesetParagraphVisualRunCount(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? 0u : paragraph->visual_runs.count;
}

const ZtypesetVisualRun* ztypesetParagraphVisualRuns(
    const ZtypesetParagraph* paragraph) {
  if (paragraph == NULL) return NULL;
  return (const ZtypesetVisualRun*)paragraph->visual_runs.data;
}

size_t ztypesetParagraphScriptRunCount(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? 0u : paragraph->script_runs.count;
}

const ZtypesetScriptRun* ztypesetParagraphScriptRuns(
    const ZtypesetParagraph* paragraph) {
  if (paragraph == NULL) return NULL;
  return (const ZtypesetScriptRun*)paragraph->script_runs.data;
}

size_t ztypesetParagraphShapingRunCount(const ZtypesetParagraph* paragraph) {
  return paragraph == NULL ? 0u : paragraph->shaping_runs.count;
}

const ZtypesetShapingRun* ztypesetParagraphShapingRuns(
    const ZtypesetParagraph* paragraph) {
  if (paragraph == NULL) return NULL;
  return (const ZtypesetShapingRun*)paragraph->shaping_runs.data;
}
...
ZtypesetResult ztypesetLineCreate(const ZtypesetParagraph* paragraph,
                                  size_t offset,
                            size_t length, ZtypesetLine** out) {
  if (out == NULL) return ZTYPESET_RESULT_INVALID_ARGUMENT;
  *out = NULL;
  if (paragraph == NULL) return ZTYPESET_RESULT_INVALID_ARGUMENT;

  // Written as a subtraction so it cannot overflow, whatever the caller passed.
  if (offset > paragraph->length) return ZTYPESET_RESULT_INVALID_ARGUMENT;
  if (length > paragraph->length - offset) return
  ...
  if (ztypesetTextSplitsCharacter(paragraph->text, paragraph->length,
                               paragraph->encoding, offset) ||
      ztypesetTextSplitsCharacter(paragraph->text, paragraph->length,
                               paragraph->encoding, offset + length)) {
    return ZTYPESET_RESULT_INVALID_ARGUMENT;
  }

  ZtypesetLine* line = ZTYPESET_NEW(ZtypesetLine);
  if (line == NULL) return ZTYPESET_RESULT_OUT_OF_MEMORY;
  line->offset = offset;
  line->length = length;
  ...
  const ZtypesetResult installed = ztypesetInstallSheenbidiAllocator();
  if (installed != ZTYPESET_RESULT_OK) {
    ztypesetLineDestroy(line);
    return installed;
  }

  const ZtypesetResult result = runsForRange(
      ztypesetAllocatorOf(line), paragraph->sb_paragraph, offset, length,
      (const ZtypesetScriptRun*)paragraph->script_runs.data,
...
void ztypesetLineDestroy(ZtypesetLine* line) {
  if (line == NULL) return;
  const ZtypesetAllocatorId owner = ztypesetAllocatorOf(line);
  ztypesetArrayFree(owner, &line->visual_runs, sizeof(ZtypesetVisualRun));
  ztypesetArrayFree(owner, &line->shaping_runs, sizeof(ZtypesetShapingRun));
  ztypesetFree(line);
}

size_t ztypesetLineOffset(const ZtypesetLine* line) {
  return line == NULL ? 0u : line->offset;
}

size_t ztypesetLineLength(const ZtypesetLine* line) {
  return line == NULL ? 0u : line->length;
}

size_t ztypesetLineVisualRunCount(const ZtypesetLine* line) {
  return line == NULL ? 0u : line->visual_runs.count;
}

const ZtypesetVisualRun* ztypesetLineVisualRuns(const ZtypesetLine* line) {
  if (line == NULL) return NULL;
  return (const ZtypesetVisualRun*)line->visual_runs.data;
}

size_t ztypesetLineShapingRunCount(const ZtypesetLine* line) {
  return line == NULL ? 0u : line->shaping_runs.count;
}

const ZtypesetShapingRun* ztypesetLineShapingRuns(const ZtypesetLine* line) {
  if (line == NULL) return NULL;
  return (const ZtypesetShapingRun*)line->shaping_runs.data;
}
